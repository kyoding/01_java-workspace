...
public class InetRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * * IP주소 : 네트워크 상에 각 컴퓨터들을 식별해줄 수 있는 번호 (컴퓨터 주민번호)
		 * 
		 * * 서버와 클라이언트
		 * > 서버 : 클라이언트(고객)에게 서비스를 제공해주는 프로그램(클라이언트의 요청을 처리해서 응답해주는) :
		 * > 클라이언트 : 서버에 요청하는 컴퓨터(서비스를 제공받는 고객)
		 * 
		 * - 서버에 요청하기 위해서는 그 요청하고자 하는 서버의 IP 주소(또는 도메인명), 포트번호 알아야됌!
		 * 
		 */
		...
		try {
			...
			InetAddress googleHost = InetAddress.getByName("www.naver.com"); // 도메인명을 통해서 해당 호스트에 대한 정보 반환
		 ...
		 *    
		 *  * 자바만을 가지고 서버와 클라이언트의 간단한 통신
		 *    이때 데이터를 입출력하고자 한다면 서버 - 클라이언트 간에 스트림(연결통로)이 필요함!!
		 *    
		 *  * 소켓 : 프로세스간의 통신을 담당하는 것! (스트림을 연결하기 위한 문)
		 *  
		 *  * 소켓 프로그래밍 (TCP 방식 / UDP 방식)
		 *  - TCP방식 : 데이터 전송 속도가 느림! 데이터가 정확하고 안정적으로 전달 가능! (신뢰성이 요구되는 프로그램 : 이메일, 은행어플)
		 *  - UDP방식 : 데이터 전송 속도가 빠름! 신뢰성 없는 데이터 전달될 수 있음 (데이터를 빠른속도로 전송하고자 하는 프로그램 : 실시간 동영상 서비스)
		 *  
		 */
