...
public class InheritRun /*extends Object*/ {
	...
	public static void main(String[] args) {
		...
		// 2. 향상된 for문
		for (Vehicle v : list) { // v = list.get(0), list.get(1) ... 반복됌
		 ...
		 *  - 자식 객체(c, s, a)는 부모클래스(Car, Ship, Airplane) 에 있는 메소드(information, hotToMove)를 마치 내것처럼 호출 가능
		 *  	+ 부모클래스에 있는 메소드가 맘에 안들면 자식클래스에서 오버라이딩 가능(내 입맛대로 재정의)
		 *  - 명시되어있지는 않지만 모든 클래스는(제공하는 클래스, 직접만드는 클래스)는 Object클래스의 후손이다.
