...
public class ListRun {
	 ...
	 *  
	 *  > 배열의 단점
	 *    1. 배열은 우선 크기를 지정해야됌!! 한번 지정된 크기를 변경할 수 없음!!
	 *    	 새로운 값을 더 추가하고자 한다면 새로운 크기의 배열을 만들고 기존꺼를 복사하는 코드를 직접 기술 
	 *    
	 *    2. 배열 중간 위치에 추가한다거나 삭제하는 경우 매번 땡겨주는 복잡한 알고리즘을 직접 기술해야됌.. 
	 *    
	...
	public static void main(String[] args) {
		...
		// 중간위치에 데이터 추가시 복잡한 알고리즘 구현 안함 => 장점2
		
		System.out.println(list);
		
		// 3. remove(int index) : 해당 인덱스 위치에 데이터 삭제시켜주는 메소드
		list.remove(1);
		...
		System.out.println(((Music)(list.get(1))).getTitle()); // Object 타입이기 때문에 강제 형변환 해줘야 한다.
		
		// 7. subList(int index1, int index2) : List => 추출해서 새로운 List 반환 : substring 같은 개념.
		List sub = list.subList(0, 3); // 반환형이 List 이기 때문
