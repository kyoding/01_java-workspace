...
public class SetRun {
	...
	public static void main(String[] args) {
		...
		// Object equals()   => 두 객체의 "주소값"을 비교해서 일치하면 true, 일치하지않으면 false를 반환
		...
		System.out.println(hs1); // 저장 순서 유지 안됌! (인덱스 개념 없음), 중복된 데이터(동일객체) 보관 불가능
		...
		System.out.println(hs2); // 저장 순서 유지안됌!! (인덱스 개념 없음) / 중복제거 안됌 => 왜? 동일객체로 판단이 안되고 있어서..!
		
		// HashSet 이라는 공간에 객체가 추가될 때 마다 동일객체 인지 비교!
		// 동일객체 : 각 객체마다 hashCode() 호출 결과가 일치하고, equals() 비교시 true일 경우!
		
		System.out.println(new Student("공유", 46, 100).hashCode());
		System.out.println(new Student("공유", 46, 100).hashCode());
		//재정의한 hashCode() 메소드에 의해 둘다 결과 똑같음 => hashCode() 메소드 주석 후 확인해볼것!
		
		System.out.println(new Student("공유", 46, 100).equals((new Student("공유", 46, 100))));
		
		// hs2.get(); => 인덱스의 개념도 없고 get메소드 자체가 정의되어있지 않음! (한 객체만 뽑아올 수 없음!!)
		
		// HashSet에 담긴 모든 객체들에 순차적으로 접근
		// 1. for문 사용 가능 (단, 향상된 for문(for each문) 만 가능!!)
		
		for(Student s : hs2) { // 제네릭 설정 안하면 Object형으로 선언해야함
		...
		//it.next(); 더 이상 뽑을 요소가 없음 java.util.NoSuchElementException 발생
